...
def getPlots(reply, start = None, stop = None):
    # Unlike previous version of this script, reply can also be a list of named parameters rather than their indices
...
#                if mo == None: break
                ...
                pi = mo.group(2)
                icount = icount + 1
                if pi in gDbNames: # Modification to allow you to specify names as well as ID numbers
                    index = gDbNames.index(pi)
                else:
                    try:
                        index = int(pi)
                    except:
...
# For the ccs-cr system, the following items are available
# 0: ImageState / length
# 1: ImageState / tag
# 2: MonitorState / goodChans
# 3: MonitorState / highLimitChange
# 4: MonitorState / lowLimitChange
# 5: MonitorState / onlineChans
# 6: RaftState / tickMillis
# 7: runtimeInfo / freeMemory
# 8: runtimeInfo / maxMemory
# 9: runtimeInfo / nThreads
# 10: runtimeInfo / processCpuLoad
# 11: runtimeInfo / systemCpuLoad
# 12: runtimeInfo / totalMemory
# 13: runtimeInfo / uptime
# 14: Scan plots / 0
# 15: Scan plots / 1
# 16: Scan plots / 10
# 17: Scan plots / 11
